...
def C_Paciorek(x_all:jax.Array, y_all:jax.Array):
    ...
    std = jnp.where(std == 0, _eps, std)
    # Scale x_all by std
    x_a, x_b = x_all.min(axis = 0), x_all.max(axis = 0)
    x_all_scaled = 2 * (x_all - x_a) / (x_b - x_a) - 1
    x_all_scaled = x_all_scaled * std.max()
    # Crossvariance
    ...
    C = ((2 * lscale_prod) / lscale_sqsum) ** (1 / 2) * jnp.exp(-covarg**2 / lscale_sqsum) * crossvar
    return C